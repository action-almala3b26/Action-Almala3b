...
function buildDeck() {
  ...
  // imgIdx counts per zone_number to round-robin images
  const imgCounters = {};
...
io.on('connection', (socket) => {
  ...
  socket.on('reset_room', () => {
    ...
    if (!room) return;
    if (room.players[0].socketId !== socket.id) {
      return socket.emit('error', { msg: 'صاحب الغرفة بس اللي يقدر يعيد اللعبة!' });
    }
    room.state = 'waiting';
    room.deck = [];
    room.turn = 0;
  ...
  socket.on('end_turn', () => {
    ...
    player.loanedCards.forEach(l => {
      if (l.turns === 0) {
        // نشيل الكارت من ملعب المستعير — لو مش موجود (اتطرد) ميرجعش لصاحبه
        let found = false;
        ...
        // لو الكارت اتلاقى في الملعب، يرجع لصاحبه — لو اتطرد أو اتحرق ميرجعش
        if (found) {
  ...
  socket.on('play_special', ({ cardId, targetPlayerIdx, targetCardId, targetZone, myCardId }) => {
    ...
    if (card.name === 'Contract' || card.name === 'Loan') {
      ...
      // تحديد المركز اللي هيروح فيه اللاعب
    ...
    // تحسب الكارت الخاص كلعبة في الدور — بعد ما الفحوصات اتعملت
    player._placedThisTurn = (player._placedThisTurn || 0) + 1;
    if (player._placedThisTurn > 2) {
      player._placedThisTurn--;
      return socket.emit('error', { msg: 'ممكن تلعب 2 كروت بس في الدور الواحد!' });
  ...
  // BROADCAST SPECIAL PLAYED — لمزامنة الكروت الخاصة في منطقة المحروق (2 لاعبين)
  socket.on('broadcast_special_played', ({ card }) => {
    const code = socket.data.roomCode;
    if (!code) return;
    // ابعت لكل اللاعبين في الأوضة ما عدا المُرسِل
    socket.to(code).emit('special_played_broadcast', { card });
  });
